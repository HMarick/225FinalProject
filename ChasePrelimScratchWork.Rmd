...
Sturges and Scott produce similar histograms and appear to be the best choice in portraying the shape of the distribution. The default contains less bins and isn't a bad choice either. However, Friedman's produces a histogram with many bins and results in a histogram that is too blocky since it doesn't have enough observations per bin.
...
The density plot created using the box, or uniform kernel, is the least smooth out of the bunch. Plots constructed with the epanechnikov, triangular, and normal kernels are all fairly smooth, but the plot using the normal kernel stands out as the most smooth so that is the one we will prefer.
